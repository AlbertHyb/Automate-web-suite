...
class Config:
    """Clase de configuración para obtener credenciales y datos de prueba."""
    
    @staticmethod
    def get_auth_credentials():
        """Retorna las credenciales de autenticación desde variables de entorno."""
        return {
    ...
    @staticmethod
    def get_signup_status_data():
        """Retorna datos base para signup de usuarios de prueba."""
        return {
...
def configure_logging():
    ...
    logging.getLogger(__name__).info("Logging básico inicializado nivel=%s archivo=%s", level_name, log_file)
...
def admin_token() -> str:
    ...
    try:
        ...
        pytest.fail(f"Timeout (30s) al intentar login en {BASE_URL}/auth/login")
    except requests.ConnectionError as e:
        pytest.fail(f"Error de conexión al intentar login: {str(e)}")
    except (ValueError, requests.exceptions.JSONDecodeError) as e:
        pytest.fail(f"Respuesta no es JSON válido: {str(e)}")
    except requests.RequestException as e:
        pytest.fail(f"Error en request de login: {str(e)}")
...
def test_user(api_client):
    ...
    unique_email = f"jose{int(time.time() * 1000)}@gmail.com"
