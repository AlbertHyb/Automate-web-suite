...
def list_aircraft_request(params, api_client, auth_headers=None):
    ...
    if clean_params != params:
        logger.debug(
            f"Parámetros ajustados: antes={params} después={clean_params}")
    params = clean_params
    ...
    try:
        ...
        if response.status_code == 200:
            logger.info(
                f"[AIRCRAFT_LIST_SUCCESS] skip={skip} limit={limit} status={response.status_code} t={duration:.3f}s")
            ...
            try:
                ...
                detail = error_data.get('detail', 'sin detalle') if isinstance(
                    error_data, dict) else 'estructura no dict'
            except Exception:
...
@pytest.mark.parametrize(
    ...
    [
        ("AIRCRAFT_LIST_C001", "Critical", {
         "skip": 0, "limit": 10}, 200, "Listar primeros 10 aviones"),
        ("AIRCRAFT_LIST_C002", "Critical", {
         "skip": 5, "limit": 10}, 200, "Listar 10 aviones saltando 5"),
        ("AIRCRAFT_LIST_C003", "Critical", {
         "skip": 0, "limit": 20}, 200, "Listar primeros 20 aviones"),
        ("AIRCRAFT_LIST_C004", "Critical", {
         "skip": 10, "limit": 5}, 200, "Listar 5 aviones desde offset 10"),
        ("AIRCRAFT_LIST_M001", "Medium", {
         "skip": -1, "limit": 10}, 422, "Skip negativo (error validación API)"),
        ("AIRCRAFT_LIST_L001", "Low", {
         "skip": 9999, "limit": 10}, 200, "Skip grande (lista vacía esperada)"),
    ]
)
def test_list_aircraft_parametrized(test_id, priority, params, expected_status, description, api_client, auth_headers):
    ...
    if status == 200:
        ...
        if "limit" in params and isinstance(params["limit"], int) and params["limit"] > 0:
            assert len(
                data) <= params["limit"], "Se excedió el límite solicitado"
        if data:
            first = data[0]
            for field in ("id", "tail_number", "model", "capacity"):
                assert field in first, f"Falta campo '{field}' en item"
        logger.info(
            f"[TEST_SUCCESS] {test_id} status={status} t={elapsed:.3f}s items={len(data)}")
    elif status in KNOWN_BACKEND_DEFECT_STATUSES:
        logger.info(
            f"[TEST_PASS_WITH_DEFECT] {test_id} status={status} t={elapsed:.3f}s (defecto conocido documentado)")
    else:
        logger.info(
            f"[TEST_EXPECTED_ERROR] {test_id} status={status} t={elapsed:.3f}s")
