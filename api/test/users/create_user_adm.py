...
def test_create_user_as_admin(admin_api_client):
    ...
    try:
        ...
        if check_response.status_code == HTTPStatus.OK:
            ...
            if delete_response.status_code in [HTTPStatus.OK, HTTPStatus.NO_CONTENT]:
                ...
                pytest.skip(
                    f"No se pudo eliminar el usuario existente: {delete_response.text}")
    ...
    print(
        f"Response: {json.dumps(response.json(), indent=2) if response.text else 'No response body'}")
    ...
    try:
        validate(instance=response.json(),
                 schema=create_user_adm_success_schema)
