...
def test_delete_user_success(api_client, auth_signup):
    ...
    if resp.status_code != HTTPStatus.NO_CONTENT:
        logger.error(
            f"FALLA: Se esperaba status 204 al eliminar usuario, pero se recibió {resp.status_code}. Respuesta: {resp.text}")
    assert resp.status_code == HTTPStatus.NO_CONTENT
    # FastAPI normalmente retorna cuerpo vacío en 204
    if resp.text not in ("", None):
        logger.error(
            f"FALLA: Se esperaba cuerpo vacío en respuesta 204, pero se recibió: {resp.text}")
...
def test_delete_user_invalid_id(api_client, admin_token, invalid_id):
    ...
    if resp.status_code not in (HTTPStatus.UNPROCESSABLE_ENTITY, HTTPStatus.BAD_REQUEST):
        logger.error(
            f"FALLA: Se esperaba status 422 o 400 al eliminar usuario con id inválido '{invalid_str}', pero se recibió {resp.status_code}. Respuesta: {resp.text}")
    assert resp.status_code in (
        HTTPStatus.UNPROCESSABLE_ENTITY, HTTPStatus.BAD_REQUEST)
    body = resp.json()
    if "detail" not in body:
        logger.error(
            f"FALLA: La respuesta no contiene el campo 'detail' para id inválido '{invalid_str}'. Respuesta: {resp.text}")
...
def test_delete_user_not_found(api_client, admin_token):
    ...
    if resp.status_code not in (HTTPStatus.NOT_FOUND, HTTPStatus.NO_CONTENT, HTTPStatus.BAD_REQUEST):
        logger.error(
            f"FALLA: Se esperaba status 404, 204 o 400 al eliminar usuario inexistente, pero se recibió {resp.status_code}. Respuesta: {resp.text}")
    assert resp.status_code in (
        HTTPStatus.NOT_FOUND, HTTPStatus.NO_CONTENT, HTTPStatus.BAD_REQUEST)
