...
def create_airport_by_iata(airport_data, api_client):
    ...
    logger.debug(
        f"Enviando solicitud POST para crear aeropuerto con IATA={iata_code}")
    ...
    if test_id == "DELETE_C001":
        try:
            ...
            if response.status_code == 200 and len(response.json()) > 0:
                ...
                logger.info(
                    f"Usando aeropuerto existente con IATA={iata_code}")
            else:
                # Si no hay aeropuertos existentes, crear uno nuevo
                logger.warning(
                    "No se encontraron aeropuertos existentes, creando uno nuevo...")
                airport_data = {"iata_code": "TYB",
                                "city": "Test City", "country": "Test Country"}
                create_response = create_airport_by_iata(
                    airport_data, api_client)
                if create_response.status_code == 201:
                    iata_code = "TYB"
                else:
                    logger.error(
                        f"No se pudo crear aeropuerto: {create_response.text}")
                    pytest.skip(
                        "No se pudo preparar la prueba: no hay aeropuertos disponibles")
        except Exception as e:
            logger.warning(
                f"Error al obtener aeropuertos existentes: {str(e)}")
            # Como fallback, intentamos con el código predefinido
            iata_code = "TYB"
            airport_data = {"iata_code": iata_code,
                            "city": "Test City", "country": "Test Country"}
