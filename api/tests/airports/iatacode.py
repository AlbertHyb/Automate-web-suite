...
import logging
import traceback
from api.tests.conftest import logger, airport_test_data  # Importamos el fixture desde conftest.py
...
def test_multiple_valid_iata_codes(iata_index, airport_test_data, api_client):
    ...
    if len(airport_test_data["airports"]) <= iata_index:
        ...
        logger.warning(f"[TEST_SKIP] Razón: {reason} | Total aeropuertos disponibles: {len(airport_test_data['airports'])}")
        pytest.skip(reason)
